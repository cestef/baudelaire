...
The repository ships four, in `themes/`, one per kind of site: `albatros` for a
blog, `spleen` for a blog with no JavaScript at all, `phares` for documentation,
`paysage` for a portfolio. The #link("/themes/")[themes page] has a live demo of
each and the three steps to get one running.
...
the nav is derived from #link("typst-modules.typ")[`@baudelaire/sections`] and
`@baudelaire/pages`, so it follows the site instead of a list in config.
