...
- [x] **un-paginated listings** — a collection's index page (`/{collection}/`) now generates from a `list` template *alone*, all members on one page; `paginate=N` became the optional split-into-chunks modifier on top. `pagination.rs` `Section::of` returns `None` only when neither is set; `per_page` defaults to the whole membership. Previously `list` without `paginate` was inert config. e2e `list_without_paginate_makes_a_single_index`; www features switched off pagination (single `/features/` index), pagination doc + title generalised to "Listings & pagination".
- [x] **configurable pagination prefix** — the `page` segment in `/{id}/page/{n}/` is a per-collection `prefix` key (default `"page"`); `prefix=""` drops the segment (`/blog/2/`). `CollectionConfig.prefix: String`, threaded to `Section` (`url`/`slug` handle empty via a `page` slug fallback). e2e `pagination_prefix_is_configurable` (custom + empty).
