...
//!
//! One pass today, [`Links`], which either fails the build or downgrades to a
//! [`Ui`] warning per its own config gate: the strict-vs-lenient policy lives
//! with the check rather than the caller. There was a `Check` trait and a
//! registry around this single impl; a trait with one implementation is not an
//! abstraction, so it is a plain call until a second validation earns one back
//! (see [`super::process::Processors`] for the shape to restore).
