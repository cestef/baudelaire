...
/// Any scheme is the author's business: `url` is served to readers, not a host
...
/// `permalink` is held to is theirs too.
#[test]
...
/// A feed's file name is joined onto `dist` by the emitter, so it is held to
/// the containment rule every other generated file name is.
#[test]
...
/// as URIs, and slugs carry raw UTF-8.
#[test]
...
/// Decoding is its inverse, and is what the dev server applies to an incoming
/// request path.
#[test]
...
/// A `dist` that contains the sources points the prune sweep at the project
/// itself.
#[test]
...
/// The guard refuses containment, not any particular location.
#[test]
...
/// One old path forwards to one place.
#[test]
