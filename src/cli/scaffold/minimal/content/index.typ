...
dictionary, compiles the rest to HTML, and hands both to the template named in
`template`. `title` is the only key this page uses; `date`, `slug`, `tags`,
`draft` and `permalink` are understood when you need them, and any key
Baudelaire does not claim is passed through to your template untouched.
