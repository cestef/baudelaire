//! Generated list pages: a titled page of links with optional prev/next
//! navigation, lowered to a synthetic [`Page`] whose body is generated typst.
...
/// One link row: a labelled link with an optional date, an optional trailing
/// note (a taxonomy member count), and the source page's extra frontmatter for
/// the template to draw on.
pub struct Item {
    url: String,
    label: String,
    /// The collection the page belongs to; empty for a row with no page behind
    /// it, as a term index's rows are.
    collection: String,
    /// The page's language code, empty for the same reason as `collection`.
    lang: String,
    date: Option<String>,
    /// The same date written the way the page's language writes one, while
    /// `date` stays the ISO-8601 day a `<time datetime>` wants.
    display: Option<String>,
    note: Option<String>,
    /// The page's one-line summary, resolved once by [`Frontmatter::blurb`].
    description: Option<String>,
    /// The page's own social image, the one the `og:image` tag names.
    image: Option<String>,
    alt: Option<String>,
    /// Who wrote the page: its own author only, never the site-wide default.
    author: Option<String>,
    /// The page's taxonomy terms, keyed by taxonomy, never flattened.
...
impl Item {
    ...
    /// The listing row for a page: the single page-to-item mapping, so every
    /// listing and every catalogue exposes the same data.
...
impl Listing {
    ...
    /// opposed to a term or a numbered page.
    ...
    /// Lower to a synthetic [`Page`] ready for the compile pipeline; its
    /// identity is namespaced by language, and its source path is absolute so
    /// the world can virtualize it, though no such file exists.
