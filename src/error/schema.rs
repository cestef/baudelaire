//! Frontmatter that does not satisfy its collection's declared schema, each
//! carrying the page source and a span so the offending line is underlined.
...
pub struct SchemaError {
    ...
    /// `None` when the frontmatter is not a literal a dialect can locate, which
    /// suppresses the snippet rather than pointing at an arbitrary offset.
...
impl SchemaError {
    ...
    ///
    /// The two arms build whole strings rather than splicing a phrase into one,
    /// because a `markup!` argument is escaped on the way in.
    pub fn missing(
        page: &std::path::Path,
        source: &str,
        span: Option<SourceSpan>,
...
enum SchemaErrorKind {
    ...
    // "of type `x`" rather than "a {x}": the type name comes from typst, so no
    // article this side picks would be right for all of them.
    #[error(
...
impl SchemaErrorKind {
    /// Constant, never interpolated: a label is rendered raw, so an authored
    /// value in it would reach the output unescaped.
