...
fn invalid_glob_reports_error() {
    ...
    site.write("content/a/p.typ", "#frontmatter((title: \"P\",))\np");
    let err = discover(&site.config()).unwrap_err();
    assert!(
...
fn page_loads_frontmatter_and_body() {
    ...
    let page = Page::load("posts", &path, &cfg).unwrap();
    ...
    assert!(!page.body.contains("frontmatter"));
}
...
fn page_without_frontmatter_loads() {
    ...
    let page = Page::load("notes", &path, &cfg).unwrap();
